...
LB66B:  ld      hl,(LB54F+1)    ; reference not aligned to instruction
        ld      a,(LB54F+1)     ; reference not aligned to instruction
        add     a,a
        ld      b,a
...
LBF55:  call    LBFAE
        ...
        ; --- START PROC LBF6A ---
LBF6A:  ld      de,(LB54F+1)    ; reference not aligned to instruction
...
LC12F:  inc     hl
        inc     hl
        call    0x5ACA
        nop                    ; PATCH - skip check for 2byte chars
        nop                 
        nop
        nop
        nop
...
LC158:  inc     hl
        ...
        ld      (0xB40C),de    ; PATCH - new variable location vPrintPos
        ld      a,(hl)
        cp      0x2C            ; ','
        ret     nz
        call    LB3D1
        ld      a,(hl)
        ld      (0xB41D),a     ; PATCH - new variable location vCharCount
        inc     hl
        call    LB3DA
...
LC16F:  inc     hl
        ...
        ld      (0xB41B),a        ; PATCH - new variable location vpatch5
        inc     hl
        ld      a,(hl)
        cp      0x2C            ; ','
        jr      z,LC185
...
LC94D:  out     (0x32),a        ; '2'
        ld      a,0x28          ; '('
        call    LCA0C          ; PATCH - OPNA
        xor     a
        add     a,d
        call    LCA16          ; PATCH - OPNA
        push    hl
        inc     hl
        ld      a,0x30          ; '0'
...
LC95D:  push    af
        add     a,d
        call   0x9980          ; PATCH - OPNA
        ld      a,(hl)
        call    0x99C2          ; PATCH - OPNA
        inc     hl
        pop     af
        add     a,0x04
        cp      0xA0
